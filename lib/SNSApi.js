...
SNSApi.postAnImageToFB = function(fieldName,msg,req,callback){
  ...
  Uploader.read(req,function(fileData){
    ...
    reqModule = request.post('https://graph.facebook.com/me/photos?access_token='+config.fb_access_token, function (err, response, body) {
      //console.log(body)
      if(body == undefined){
        return callback({
          error: {message: [{msg: 'unexpected errors please try again', type: 'danger'}]},
