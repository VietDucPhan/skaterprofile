...
Socket.connections = function (socket) {
  ...
  socket.on('notification',function(data){
    ...
    if(data && data.post_data && data.post_data.posted_to_alias && data.post_data.posted_by_alias){
      if(data.post_data.posted_to_alias._id == data.post_data.posted_by_alias._id){
        socket.to(data.post_data.posted_by_alias.admin).emit('notification', data);
      } else {
        socket.to(data.post_data.posted_by_alias.admin).emit('notification', data);
        socket.to(data.post_data.posted_to_alias.admin).emit('notification', data);
      }
    }

    if(data && data.follower_admin_id){
      socket.to(data.follower_admin_id).emit('notification', data);
    }
  });
