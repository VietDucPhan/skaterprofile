  ...
  async.waterfall([
    function (callback) {
      v.isEmail(data.email, function (flag, email) {
        if (!flag) {
          error.push({msg: 'Please input a valid email', type: 'warning'});
        } else {
          //error = null;
    ...
    function (error, safeData, callback) {
      v.isEmailExisted(safeData.email, function (flag, email) {
        //console.log(flag);
        if (flag) {
          error.push({msg: 'Email already in used', type: 'warning'});
        }
        callback(null, error, safeData);
      });
    },
    function (error, safeData, callback) {
      v.isValidPassword(data.password, function (flag, password) {
        ...
        if (!flag) {
          error.push({msg: 'Password must be more than 6 characters', type: 'warning'});
          callback(null, error, safeData);
        } else {
...
v.isUsernameExist = function (username, callback) {
  ...
  var flag = false;
  alias.find({username: username}).limit(1).toArray(function (err, explanation) {
