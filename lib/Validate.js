...
v.isValidUsername = function (text, callback) {
  ...
  var flag = false;
  var errMsg = [];
  var notContainWords = ['dashboard','hot', 'skaters', 'companies', 'tricks', 'spots']
  if (RegExp.test(text) && notContainWords.indexOf(text.toLowerCase()) == -1) {

    if(text.length < 24){
      flag = true;
    } else {
      errMsg.push({msg:"Username must be between 6 and 24 characters", type:'warning'});
    }

  } else {
    errMsg.push({msg:"Only use letters, numbers, '_' and Can not contain main menu words", type:'warning'});
  }

  if (typeof callback == 'function') {
    return callback(flag, text, errMsg);
  }
  return flag;
};
...
v.getVideoId = function(url,type,callback){
  ...
  request(url, function (error, response, html) {
    if (!error && response.statusCode == 200) {
      ...
      var videoImage = $('meta[property="og:image"]').attr('content');
      var videoId;
      ...
      if(typeof callback == 'function'){
        return callback(videoId,videoImage)
      }
